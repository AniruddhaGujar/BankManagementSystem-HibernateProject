...
//	get customer by id
	public Customer getCustomerById(int id) {
		...
		if (customer != null) {
			...
			System.out.println("Customer ID dont exist");
		return customer;
	}
	...
	public Account saveAccount(int customer_id, Account account) {
		...
		if (customer != null) {
			if (customer.getStatus().equals("approved")) {
				account.setAccountNo(123456789 + customer.getId());
				account.setIfsc("BKID008808");
				...
				System.out.println("You are not an approved Customer");
		} else
	...
	public Account depositeMoney(int customer_id, int account_id, double amount) {
		...
		if (customer != null) {
			if (account != null) {
				...
				System.out.println(account_id + " is invalid Customer's Account Id");
		} else
			System.out.println(customer_id + " is invalid Customer Id");
		return account;
	}
		...
		if (customer.getStatus().equals("approved") && senderAccount.getCustomer().getId() == customer_id) {
			if (senderAccount != null && receiverAccount != null) {
				if (senderAccount.getBalance() >= transferAmount) {
					...
					System.out.println("Account balance should be greater than transfer maount");
			} else
